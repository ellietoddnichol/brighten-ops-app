...
export default function ProjectDetailPage() {
  ...
  async function loadData() {
    ...
    setError(null)
    try {
      const [projectData, quoteData, vendorData] = await Promise.all([
  ...
  return (
    <div className="page">
      <header className="page-header">
        ...
        <Link className="button-link" to={`/projects/${project.id}/estimate`}>
          Estimate Review
        </Link>
      </header>
      ...
      <section className="card">
        ...
        <form className="form-inline" onSubmit={handleCreateVendor}>
          <input
            placeholder="Vendor name (e.g. Bradley)"
            value={newVendorName}
      ...
      <section className="card">
        <h2>Vendor Quotes</h2>
        {quotes.length === 0 ? (
          <p className="muted">No quotes yet. Create one to start manual line entry.</p>
        ) : (
